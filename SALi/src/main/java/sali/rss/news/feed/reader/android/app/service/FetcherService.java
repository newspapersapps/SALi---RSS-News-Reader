package sali.rss.news.feed.reader.android.app.service;
...
import de.jetwick.snacktory.ArticleTextExtractorRss;
...
import sali.rss.news.feed.reader.android.app.activity.HomeActivity;
import sali.rss.news.feed.reader.android.app.parser.RssAtomParser;
...
public class FetcherService extends IntentService {
    ...
    public static final String ACTION_REFRESH_FEEDS = "sali.rss.news.feed.reader.android.app.REFRESH";
    public static final String ACTION_MOBILIZE_FEEDS = "sali.rss.news.feed.reader.android.app.MOBILIZE_FEEDS";
    public static final String ACTION_DOWNLOAD_IMAGES = "sali.rss.news.feed.reader.android.app.DOWNLOAD_IMAGES";
                    ...
                    if (newCount > 0) {
                        ...
                        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                            ...
                            Notification.Builder notifBuilder = new Notification.Builder(MainApplication.getContext()) //
                                    ...
                                    .setAutoCancel(true) //
                                    .setContentTitle(getString(R.string.sali_feeds)) //
                                    .setContentText(text) //
                            ...
                            NotificationChannel notificationChannel = new NotificationChannel("0", "feeds", NotificationManager.IMPORTANCE_LOW);
                            notificationChannel.enableLights(PrefUtils.getBoolean(PrefUtils.NOTIFICATIONS_LIGHT, false));
                            notificationChannel.setLightColor(0xffffffff);
                            notificationChannel.enableVibration(PrefUtils.getBoolean(PrefUtils.NOTIFICATIONS_VIBRATE, false));
                            notificationChannel.setVibrationPattern(new long[]{0, 1000});
                            String ringtone = PrefUtils.getString(PrefUtils.NOTIFICATIONS_RINGTONE, null);
                            if (ringtone != null && ringtone.length() > 0) {
                                notificationChannel.setSound(Uri.parse(ringtone),
                            ...
                            Notification.Builder notificationBuilder =
                                    new Notification.Builder(MainApplication.getContext(), "0")
                                            .setContentIntent(contentIntent) //
                                            .setSmallIcon(R.drawable.ic_statusbar_rss) //
                                            .setLargeIcon(BitmapFactory.decodeResource(getResources(), R.mipmap.ic_launcher)) //
                                            .setWhen(System.currentTimeMillis()) //
                            ...
                            if (Constants.NOTIF_MGR != null) {
                                Constants.NOTIF_MGR.createNotificationChannel(notificationChannel);
                                Constants.NOTIF_MGR.notify(0, notificationBuilder.build());
                            }
                        }
                    }
            ...
            if (entryCursor.moveToFirst()) {
                if (entryCursor.isNull(entryCursor.getColumnIndex(EntryColumns.MOBILIZED_HTML))) { // If we didn't already mobilized it
                    ...
                    try {
                        ...
                        fetcher.setExtractor(new ArticleTextExtractorRss());
                        //fetcher.setUserAgent(Constants.USER_AGENT);
                        JResult res = fetcher.fetchAndExtract(link, 5000, true);
                        String mobilizedHtml = res.getText();
                        ...
                        if (mobilizedHtml != null) {
                            ...
                            if (NetworkUtils.needDownloadPictures()) {
                                for (ImageResult img : res.getImages()) {
                                    imgUrlsToDownload.add(img.src);
                                }
                            }

                            if (res.getImageUrl() != null) {
                                values.put(EntryColumns.IMAGE_URL, res.getImageUrl());
                            }
    ...
    public static void deleteFeedEntries(long feedId, boolean onlyReadEntries) {

        // Delete the cache files
        NetworkUtils.deleteFeedImagesCache(feedId, onlyReadEntries);

        // Delete the entries
        String where = EntryColumns.WHERE_NOT_FAVORITE;
        if (onlyReadEntries) {
            where = where + Constants.DB_AND + EntryColumns.IS_READ;
        }
        MainApplication.getContext().getContentResolver().delete(EntryColumns.ENTRIES_FOR_FEED_CONTENT_URI(feedId), where, null);
    }

    public static void cleanLastSyncTime(long feedId) {
        //clean last sync time
        ContentValues values = new ContentValues();
        values.putNull(FeedColumns.LAST_UPDATE);
        values.putNull(FeedColumns.REAL_LAST_UPDATE);
        MainApplication.getContext().getContentResolver().update(FeedColumns.CONTENT_URI(feedId), values, null, null);
    }

    public static void markFeedReadStatus(long feedId, boolean isRead) {
        ContentValues values = new ContentValues();
        if (isRead)
            values.put(EntryColumns.IS_READ, true);
        else
            values.putNull(EntryColumns.IS_READ);
        MainApplication.getContext().getContentResolver().update(EntryColumns.ENTRIES_FOR_FEED_CONTENT_URI(feedId), values, null, null);
    }
    ...
    private int refreshFeed(String feedId, long keepDateBorderTime) {
        ...
        if (cursor.moveToFirst()) {
            ...
            try {
                ...
                handler = new RssAtomParser(new Date(cursor.getLong(realLastUpdatePosition)), keepDateBorderTime, id, cursor.getString(titlePosition), feedUrl, true);
